var Prioritizer = function() {
	...
	function createComponents() {
		...
		options.each(function(o, idx) {
			...
			draggable.grab(new Element("span", { text: o }));
			...
			droppable.grab(new Element("span", { text: idx + 1 }));
	...
	function stretchDraggables() {
		var totalSpacing = config.draggableSpacing
			* (draggables.length - 1);
		...
		draggables.each(function(d) { width += d.getSize().x; });
